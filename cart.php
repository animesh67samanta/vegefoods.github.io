<?php
	...
	include 'config.php';
	if (isset($_POST['pid'])) {
		$selectItem = mysqli_query($conn, "SELECT * FROM table_vegetable WHERE Vegetable_id = '$_POST[pid]' ");
		$vegetableItems = mysqli_fetch_array($selectItem);
		if($_POST['qut'] > $vegetableItems['Quantity']) { 
			$_SESSION['status'] = "You have orderd excess quantity.";
		} 
		else{

			$date = date('d-m-Y');
			$amount = ($_POST['qut'] * $vegetableItems['Price']);
			
			$cartItem = mysqli_query($conn, "INSERT INTO table_orders(User_id,Order_date,Vegetable_id,Order_status,Paid_amount,Quantity) VALUES('$_SESSION[uid]','$date','$_POST[pid]','0','$amount','$_POST[qut]')");
			$updateItem = mysqli_query($conn, "UPDATE table_vegetable SET Quantity = Quantity - '$_POST[qut]' WHERE Vegetable_id = '$_POST[pid]'");
			$updateWishlist = mysqli_query($conn, "DELETE FROM `table_wishlist` WHERE Vegetable_id = '$_POST[pid]' AND User_id = '$_SESSION[uid]'");
  	...
  	<body class="goto-here">
				...
				<div class="collapse navbar-collapse" id="ftco-nav">
					<ul class="navbar-nav ml-auto">
						...
						<?php 
							if(!empty($_SESSION['uid'])){
								include('config.php');
								$selectUser = mysqli_query($conn,"SELECT * FROM table_user WHERE User_id = '$_SESSION[uid]'");
						...
						?>       
						<?php 
							$cartItem = mysqli_query($conn, "SELECT * FROM table_orders WHERE User_id = '$_SESSION[uid]' and Order_status = '0' ");
						?>
						<li class="nav-item active cta cta-colored"><a href="cart.php" class="nav-link"><span class="icon-shopping_cart"></span>[<?php echo mysqli_num_rows($cartItem);?>]</a></li>
					</ul>
				</div>
			</div>
		</nav>
		...
		<?php
			...
			if(isset($_SESSION['status']) && $_SESSION != ''){?>
                <div class="alert alert-danger d-flex align-items-center alert-dismissible fade show" role="alert" id="cancel_order">
			...
			if(isset($_SESSION['order_success']) && $_SESSION != ''){?>
                <div class="alert alert-success d-flex align-items-center alert-dismissible fade show" role="alert" id="order_success">
                	<svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Success:"><use xlink:href="#check-circle-fill"/></svg>
                    <div class="mx-2"><?php echo $_SESSION['order_success']; ?> </div>
                    <button type="button" class="close" data-dismiss="alert">&times;</button>
                </div>
                <?php
            ...
            <div class="container">
                <div class="row no-gutters slider-text align-items-center justify-content-center">
                    <div class="col-md-9 ftco-animate text-center fadeInUp ftco-animated">
                        <p class="breadcrumbs "><span class="mr-2 display-4 text-info"><b>cart Items</b></span></p>
                    </div>
                </div>
            </div>
        </div>
							...
							<table class="table">
						    	...
						    	<tbody>
									<?php
										...
										$userOrder = mysqli_query($conn, "SELECT * FROM table_orders WHERE  User_id = '$_SESSION[uid]' ");
										...
										while ($orderItems = mysqli_fetch_array($userOrder)) {
											...
											<tr class="text-center">
												<?php 
													if ($orderItems['Order_status'] == 0) { ?>
														<td class="product-remove" data-toggle="tooltip" title="Delete Cart Item"><a class="bg-warning" href="cancel.php?oid=<?php echo $orderItems['Order_id'];?>"><span class="ion-ios-close"></span></a></td>
														<?php
													}elseif ($orderItems['Order_status'] == 1) { ?>
												...
												<?php
													if($orderItems['Order_status'] == 0){ ?>
														<td class="total"><a href="buy.php?oid=<?php echo $orderItems['Order_id'];?>" class="btn btn-primary">Buy</a></td>
														<?php 
													} 
													if($orderItems['Order_status'] == 1){ ?>
														<td class="total"> Order Dispatched...</td>
														<?php
													}
												?>
											</tr>
		...
		<script>
			...
			window.setTimeout(function() {
				$("#cancel_order").fadeTo(500, 0).slideUp(500, function(){
					$(this).remove(); 
				});
			}, 5000);

			window.setTimeout(function() {
				$("#order_success").fadeTo(500, 0).slideUp(500, function(){
					$(this).remove(); 
				});
			}, 5000);
